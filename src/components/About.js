...
const Stat = ({statName, statNumber, statWord}) => (
  <div className="w-third tc pa3 ba bw2 b--light-gray" style={{marginRight: -2}}>
    <div className="f6 rubik ttu">{statName}</div>
    <div className="f5 b rubik ttu tracked">
      {statNumber} {statWord}
    </div>
  </div>
);
...
const About = ({mixes}) => (
  <div className="ph3 ph4-l">
    ...
    <div className="flex pt3">
      ...
      <Stat
        statName="Played..."
        statNumber={mixes.reduce((acc, curr) => acc + curr.play_count, 0)}
        statWord="times"
      />
      <Stat
        statName="With..."
        statNumber={mixes.reduce((acc, curr) => acc + curr.audio_length, 0)}
        statWord="seconds"
      />
    </div>
  </div>
